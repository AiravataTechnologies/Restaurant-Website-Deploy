...
export interface IStorage {
  ...
  getSpecialItems(): Promise<SpecialItem[]>;
  getTestimonials(): Promise<Testimonial[]>;
  getGalleryImages(): Promise<GalleryImage[]>;
  createTableBooking(booking: InsertTableBooking): Promise<TableBooking>;
  sendWhatsAppNotification(booking: TableBooking): Promise<boolean>;
}
...
export class MemStorage implements IStorage {
  ...
  private tableBookings: Map<number, TableBooking>;
  ...
  async getSpecialItems(): Promise<SpecialItem[]> {
  ...
  async createTableBooking(booking: InsertTableBooking): Promise<TableBooking> {
    const id = this.bookingId++;
    const now = new Date().toISOString();
    
    // Create a properly typed TableBooking object
  ...
  async sendWhatsAppNotification(booking: TableBooking): Promise<boolean> {
    // In a real application, this would integrate with WhatsApp Business API
    // or a service like Twilio to send actual messages
    ...
    const updatedBooking = { ...booking, notificationSent: true };
    this.tableBookings.set(booking.id, updatedBooking);
