...
export function ChefsSpecial() {
  ...
  // Static special items (avoiding API issues for now)
  const specialItems = [
    {
      id: 1,
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8" ref={itemsRef}>
          {specialItems.map((item) => (
            <div 
              key={item.id}
              className="bg-black bg-opacity-30 p-6 rounded-lg text-center transition-all hover:scale-105 fade-in"
            >
              <img 
                src={item.image} 
                alt={item.name} 
        ...
        <div className="text-center mt-12 fade-in" ref={buttonRef}>
          <Button 
            ...
            className="bg-accent hover:bg-accent/90 text-white py-3 px-8 rounded-sm uppercase text-sm tracking-wider font-medium inline-block hover:text-white"
          >
            Reserve a Table
          </Button>
        </div>
