...
export function Hero() {
  return (
    <section id="home" className="relative h-screen overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-20 h-full flex flex-col justify-center items-center text-center text-white">
        <span className="font-script text-8xl md:text-9xl text-accent mb-2">Food Platform</span>
        <h1 className="font-display text-3xl md:text-5xl font-bold mb-4 max-w-4xl">
          Flavors of Purity Crafted with Passion
        </h1>
        <p className="text-neutral-200 text-lg md:text-xl mb-10 max-w-3xl">
          At Food Platform, every dish is a tribute to purity and tradition. Crafted with care, rich in flavor, and
          served with warmth, experience vegetarian dining at its finest.
        </p>
        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6">
          <Button
            onClick={() => scrollToSection('menu')}
          ...
          {/* Updated Book a Table button */}
          <Button
            onClick={() => scrollToSection('booking')}
            className="bg-transparent border-2 border-white text-white hover:bg-white hover:text-black transition-all duration-300 py-3 px-8 rounded-sm uppercase text-sm tracking-wider font-medium"
          >
            Book a Table
          </Button>
        </div>
